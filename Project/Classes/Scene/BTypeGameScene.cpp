...
//二维向量,元素舞台
std::vector<Block*> blockLine2;
std::vector<std::vector<Block*>> blockStage2;
std::vector<IceBlock*> iceBlockLine;
std::vector<std::vector<IceBlock*>> iceBlockStage;

Scene* BTypeGameScene::createScene()
{
	return BTypeGameScene::create();
}

bool BTypeGameScene::init()
{
	if (!Scene::init()) { return false; };

	sprintf(targetChar, "block/iceBlock.png");

	if (UserDefault::getInstance()->getIntegerForKey("BTypeLevel") == 1) {
		maxStep = 20;
		maxGoal = 24;
		step = maxStep;
		goal = maxGoal;
...
//静态元素
void BTypeGameScene::initStillNode()
{
...
//动态元素
void BTypeGameScene::initMoveLayer()
{
	auto vSize = Director::getInstance()->getVisibleSize();
	Vec2 origin = Director::getInstance()->getVisibleOrigin();
	...
	auto pauseItem = MenuItemImage::create("pic/PauseNormal.png", "pic/PauseSelected.png",
		CC_CALLBACK_1(BTypeGameScene::getSetUpLayer, this, moveLayer));
...
//菜单元素
void BTypeGameScene::initMenu(Node* pNode)
{
	auto vSize = Director::getInstance()->getVisibleSize();
	Vec2 origin = Director::getInstance()->getVisibleOrigin();
	...
	musicSlider->addTargetWithActionForControlEvents(this,
		cccontrol_selector(BTypeGameScene::musicSliderCallBack), Control::EventType::VALUE_CHANGED);
	...
	effectSlider->addTargetWithActionForControlEvents(this,
		cccontrol_selector(BTypeGameScene::effectSliderCallBack), Control::EventType::VALUE_CHANGED);
	pNode->addChild(effectSlider, 6);

	//动态菜单关闭按钮
	auto menuCloseItem = MenuItemImage::create("pic/MenuCloseNormal.png", "pic/MenuCloseSelected.png",
		CC_CALLBACK_1(BTypeGameScene::menuClose, this, pNode));
	...
	auto returnGameItem = MenuItemImage::create("pic/ReturnHomeNormal.png", "pic/ReturnHomeSelected.png",
		CC_CALLBACK_1(BTypeGameScene::returnCallBack, this));
	...
	auto closeItem = MenuItemImage::create("pic/CloseNormal2.png", "pic/CloseSelected2.png",
		CC_CALLBACK_1(BTypeGameScene::gameCloseCallback, this));
...
//音乐滑块效果
void BTypeGameScene::musicSliderCallBack(Ref* pSender, Control::EventType type)
{
...
//音效滑块效果
void BTypeGameScene::effectSliderCallBack(Ref* pSender, Control::EventType type)
{
	auto slider = (ControlSlider*)pSender;
	auto value = slider->getValue();
	value = value / 100;
...
//鼠标点击关闭操作
void BTypeGameScene::gameCloseCallback(Ref* pSender)
{
	blockLine2.clear();
	blockStage2.clear();
	iceBlockLine.clear();
...
//鼠标点击弹出菜单
void BTypeGameScene::getSetUpLayer(Ref* pSender, Node* pNode)
{
...
//鼠标点击关闭菜单
void BTypeGameScene::menuClose(Ref* pSender, Node* pNode)
{
...
//返回上级场景
void BTypeGameScene::returnCallBack(Ref* pSender)
{
	step = maxStep;
	goal = maxGoal;
	grade = 0;
...
/*********************************游戏逻辑部分*********************************/
void BTypeGameScene::initGameLayer()
{
	...
	auto skillItem1 = MenuItemImage::create("pic/Skill1Normal.png", "pic/Skill1Selected.png",
		CC_CALLBACK_1(BTypeGameScene::addStep, this));
	...
	auto skillItem2 = MenuItemImage::create("pic/Skill2Normal.png", "pic/Skill2Selected.png",
		CC_CALLBACK_1(BTypeGameScene::switchTwo, this));
	...
	auto skillItem3 = MenuItemImage::create("pic/Skill3Normal.png", "pic/Skill3Selected.png",
		CC_CALLBACK_1(BTypeGameScene::reFill, this));
	...
	if (UserDefault::getInstance()->getIntegerForKey("BTypeLevel") == 1) {
		for (int i = 0; i < 8; i++) {
			for (int j = 0; j < 8; j++) {
				if (i >= 1 && i <= 6 && j >= 2 && j <= 5) {
					IceBlock* pIce = new IceBlock(pos[i][j], 1);
					iceBlockLine.push_back(pIce);
					this->addChild(pIce->getSpePic(), 1);
				}
				else {
					IceBlock* pIce = new IceBlock(pos[i][j], 0);
					iceBlockLine.push_back(pIce);
					this->addChild(pIce->getSpePic(), 1);
				}
			}
			iceBlockStage.push_back(iceBlockLine);
			iceBlockLine.clear();
		}
	}
	else {
	...
	for (int i = 0; i < 8; i++) {
		...
		blockStage2.push_back(blockLine2);
		blockLine2.clear();
	}

	//刷新
	this->schedule(schedule_selector(BTypeGameScene::clearBlocks), 0.5f);
	this->schedule(schedule_selector(BTypeGameScene::explode), 1.5f);
	this->schedule(schedule_selector(BTypeGameScene::endGame), 3.7f);
	this->scheduleUpdate();
}

void BTypeGameScene::addStep(cocos2d::Ref* pSender)
{
...
void BTypeGameScene::switchTwo(cocos2d::Ref* pSender)
{
...
void BTypeGameScene::reFill(cocos2d::Ref* pSender)
{
	if (UserDefault::getInstance()->getIntegerForKey("SkillThree") > 0) {
		...
		this->unschedule(schedule_selector(BTypeGameScene::clearBlocks));

		for (int i = 0; i < 8; i++) {
			for (int j = 0; j < 8; j++) {
				blockStage2[i][j]->getPic()->runAction(ScaleTo::create(0.0f, 0.0f));
				blockStage2[i][j]->getSpePic()->runAction(ScaleTo::create(0.0f, 0.0f));
			}
		}
...
void BTypeGameScene::update(float dt)
{
	...
	//鼠标点击第1次
	if (Block::flag == 1) {
		for (int i = 0; i < blockStage2.size(); i++) {
			for (int j = 0; j < blockStage2[i].size(); j++) {
				if (blockStage2[i][j]->getSwap() == 1) {
					MoveTo* move = MoveTo::create(0, blockStage2[i][j]->getPos());
	...
	if (Block::flag == 2) {
		...
		if ((numi[0] == numi[1] && abs(numj[0] - numj[1]) == 1) || (numj[0] == numj[1] && abs(numi[0] - numi[1]) == 1)) {
			cocos2d::MoveTo* move1 = cocos2d::MoveTo::create(0.05f, blockStage2[numi[0]][numj[0]]->getPos());
			cocos2d::MoveTo* move2 = cocos2d::MoveTo::create(0.05f, blockStage2[numi[1]][numj[1]]->getPos());

			blockStage2[numi[0]][numj[0]]->getPic()->runAction(Sequence::create(move2->clone(), DelayTime::create(0.05f), NULL));
			blockStage2[numi[0]][numj[0]]->getSpePic()->runAction(Sequence::create(move2->clone(), DelayTime::create(0.05f), NULL));
			blockStage2[numi[1]][numj[1]]->getPic()->runAction(Sequence::create(move1->clone(), DelayTime::create(0.05f), NULL));
			blockStage2[numi[1]][numj[1]]->getSpePic()->runAction(Sequence::create(move1->clone(), DelayTime::create(0.05f), NULL));

			Point po = blockStage2[numi[0]][numj[0]]->getPos();
			blockStage2[numi[0]][numj[0]]->setPos(blockStage2[numi[1]][numj[1]]->getPos());
			blockStage2[numi[1]][numj[1]]->setPos(po);

			blockStage2[numi[0]][numj[0]]->initSwap();
			blockStage2[numi[1]][numj[1]]->initSwap();

			std::swap(blockStage2[numi[0]][numj[0]], blockStage2[numi[1]][numj[1]]);

			if (!clearAble(numi[0], numj[0]) && !clearAble(numi[1], numj[1]) && isSwitchOn == false) {
				cocos2d::MoveTo* move3 = cocos2d::MoveTo::create(0.05f, blockStage2[numi[0]][numj[0]]->getPos());
				cocos2d::MoveTo* move4 = cocos2d::MoveTo::create(0.05f, blockStage2[numi[1]][numj[1]]->getPos());

				blockStage2[numi[0]][numj[0]]->getPic()->runAction(Sequence::create(DelayTime::create(0.05f), move4->clone(), NULL));
				blockStage2[numi[0]][numj[0]]->getSpePic()->runAction(Sequence::create(DelayTime::create(0.05f), move4->clone(), NULL));
				blockStage2[numi[1]][numj[1]]->getPic()->runAction(Sequence::create(DelayTime::create(0.05f), move3->clone(), NULL));
				blockStage2[numi[1]][numj[1]]->getSpePic()->runAction(Sequence::create(DelayTime::create(0.05f), move3->clone(), NULL));

				po = blockStage2[numi[0]][numj[0]]->getPos();
				blockStage2[numi[0]][numj[0]]->setPos(blockStage2[numi[1]][numj[1]]->getPos());
				blockStage2[numi[1]][numj[1]]->setPos(po);
...
void BTypeGameScene::isTarget(const int x, const int y) {
	if (!(step == maxStep && isAddStep == false && goal == maxGoal)) {
		if (iceBlockStage[x][y]->getState() == 1) {
			goal = goal - 1;
			iceBlockStage[x][y]->setState(0);
			iceBlockStage[x][y]->getSpePic()->runAction(ScaleTo::create(0.1f, 0.0f));
		}
	}
}

void BTypeGameScene::clearBlocks(float dt)
{
	...
	auto clearOne = [&](int X, int Y) {
		blockStage2[X][Y]->getPic()->runAction(Sequence::create(ScaleTo::create(0.15f, 0.0f), NULL));
		blockStage2[X][Y]->getSpePic()->runAction(Sequence::create(ScaleTo::create(0.15f, 0.0f), NULL));
		isTarget(X, Y);
		choseEffect(X, Y);
		blockStage2[X][Y]->setName("null");
	};
	...
	for (int i = 0; i < 8; i++) {
		for (int j = 0; j < 8; j++) {
			...
			//判断有没有横着的(向右看)
			if (j < blockStage2[i].size() - 2) {
				if (blockStage2[i][j]->getName() == blockStage2[i][j + 1]->getName() && blockStage2[i][j]->getName() == blockStage2[i][j + 2]->getName()) {
					pushMeltPos(i, j);
					pushMeltPos(i, j + 1);
					pushMeltPos(i, j + 2);
					//4消
					if (j + 3 < blockStage2[i].size() && blockStage2[i][j]->getName() == blockStage2[i][j + 3]->getName()) {
						pushMeltPos(i, j + 3);
						//5消
						if (j + 4 < blockStage2[i].size() && blockStage2[i][j]->getName() == blockStage2[i][j + 4]->getName()) {
							pushMeltPos(i, j + 4);
						}
					}
					key = 1;
			...
			if (j - 2 >= 0) {
				if (blockStage2[i][j]->getName() == blockStage2[i][j - 1]->getName() && blockStage2[i][j]->getName() == blockStage2[i][j - 2]->getName()) {
					...
					pushMeltPos(i, j - 2);
					//4消
					if (j - 3 >= 0 && blockStage2[i][j]->getName() == blockStage2[i][j - 3]->getName()) {
						pushMeltPos(i, j - 3);
						//5消
						if (j - 4 >= 0 && blockStage2[i][j]->getName() == blockStage2[i][j - 4]->getName()) {
							pushMeltPos(i, j - 4);
						}
					}
					key = 1;
			...
			//判断有没有竖着的(向上看)
			if (i < blockStage2.size() - 2) {
				if (blockStage2[i][j]->getName() == blockStage2[i + 1][j]->getName() && blockStage2[i][j]->getName() == blockStage2[i + 2][j]->getName()) {
					...
					pushMeltPos(i + 2, j);
					//4消
					if (i + 3 < blockStage2.size() && blockStage2[i][j]->getName() == blockStage2[i + 3][j]->getName()) {
						pushMeltPos(i + 3, j);
						//5消
						if (i + 4 < blockStage2.size() && blockStage2[i][j]->getName() == blockStage2[i + 4][j]->getName()) {
							pushMeltPos(i + 4, j);
						}
					}
					key = 1;
			...
			if (i - 2 >= 0) {
				if (blockStage2[i][j]->getName() == blockStage2[i - 1][j]->getName() && blockStage2[i][j]->getName() == blockStage2[i - 2][j]->getName()) {
					...
					pushMeltPos(i - 2, j);
					//4消
					if (i - 3 >= 0 && blockStage2[i][j]->getName() == blockStage2[i - 3][j]->getName()) {
						pushMeltPos(i - 3, j);
						//5消
						if (i - 4 >= 0 && blockStage2[i][j]->getName() == blockStage2[i - 4][j]->getName()) {
							pushMeltPos(i - 4, j);
						}
					}
					key = 1;
	...
	if (maxMelti.size() != 0) {
		...
		if (maxMelti.size() == 3) {
			...
			int powerKey = 0;
			for (int i = 0; i < 3; i++) {
				if (blockStage2[maxMelti[i]][maxMeltj[i]]->getState() == 2) {
					blockStage2[maxMelti[i]][maxMeltj[i]]->setState(1);
					this->addChild(blockStage2[maxMelti[i]][maxMeltj[i]]->getSpePic(), 1);
					specialKey = 1;
				}
				if (blockStage2[maxMelti[i]][maxMeltj[i]]->getState() == 3) {
					blockStage2[maxMelti[i]][maxMeltj[i]]->setState(1);
					this->addChild(blockStage2[maxMelti[i]][maxMeltj[i]]->getSpePic(), 1);
					powerKey = 1;
				}
			...
			if (specialKey == 0 && powerKey == 0) {
				...
				for (int k = 0; k < maxMelti.size(); k++) {
					for (int i = 0; i < 8; i++) {
						if (blockStage2[maxMelti[k]][i]->getName() != "null") {
							clearOne(maxMelti[k], i);
						}
						grade += pow(2, 9);
					}
					for (int i = 0; i < 8; i++) {
						if (blockStage2[i][maxMeltj[k]]->getName() != "null") {
		...
		//4消
			...
			if (specialKey == 0 && powerKey == 0) {
				for (int k1 = 0; k1 < maxMelti.size(); k1++) {
					if (k1 == 1) {
						blockStage2[maxMelti[1]][maxMeltj[1]]->setState(2);
						this->addChild(blockStage2[maxMelti[1]][maxMeltj[1]]->getSpePic(), 1);
		...
		//5或更多消
			...
			if (specialKey == 0 && powerKey == 0) {
				for (int k1 = 0; k1 < maxMelti.size(); k1++) {
					if (k1 == 1) {
						blockStage2[maxMelti[1]][maxMeltj[1]]->setState(3);
						this->addChild(blockStage2[maxMelti[1]][maxMeltj[1]]->getSpePic(), 1);
						isTarget(maxMelti[1], maxMeltj[1]);
					}
					else {
...
void BTypeGameScene::fillFull()
{
	//向下补齐
	for (int i = 0; i < 8; i++) {
		for (int j = 0; j < 8; j++) {
			if (blockStage2[j][i]->getName() == "null") {
				for (int k2 = j; k2 < 8; k2++) {
					if (blockStage2[k2][i]->getName() == "null") {
						continue;
					}
					else {
						Point tempPos = blockStage2[k2][i]->getPos();
						blockStage2[k2][i]->setPos(blockStage2[j][i]->getPos());
						blockStage2[j][i]->setPos(tempPos);

						std::swap(blockStage2[k2][i], blockStage2[j][i]);

						blockStage2[j][i]->getPic()->runAction(
							Sequence::create(MoveTo::create(0.15f, pos[j][i]), NULL));
						blockStage2[j][i]->getSpePic()->runAction(
							Sequence::create(MoveTo::create(0.15f, pos[j][i]), NULL));
						blockStage2[k2][i]->getPic()->runAction(
							Sequence::create(MoveTo::create(0.15f, pos[k2][i]), NULL));
						blockStage2[k2][i]->getSpePic()->runAction(
							Sequence::create(MoveTo::create(0.15f, pos[j][i]), NULL));
	...
	for (int i = 0; i < 8; i++) {
		for (int j = 0; j < 8; j++) {
			if (blockStage2[i][j]->getName() == "null") {
				picNum = rand() % 6 + 1 + 6 * (UserDefault::getInstance()->getIntegerForKey("Skin") - 1);
				sprintf(picName, "block/llk%d.png", picNum);
				blockStage2[i][j]->setName(picName);
				blockStage2[i][j]->resetPic(picName, spos[j]);

				blockStage2[i][j]->getPic()->runAction(
					Sequence::create(MoveTo::create(0.15f, pos[i][j]), NULL));
				blockStage2[i][j]->getSpePic()->runAction(
					Sequence::create(MoveTo::create(0.15f, pos[i][j]), NULL));

				auto menu = Menu::create(blockStage2[i][j]->getPic(), NULL);
				menu->setPosition(Point::ZERO);
				this->addChild(menu, 1);
				this->addChild(blockStage2[i][j]->getSpePic(), 1);
			}
		}
	}
}

bool BTypeGameScene::clearAble(const int x1, const int y1)
{
	int count = 1;
	if (x1 + 1 < 8 && blockStage2[x1][y1]->getName() == blockStage2[x1 + 1][y1]->getName()) {
		count = count + 1;
		if (x1 + 2 < 8 && blockStage2[x1][y1]->getName() == blockStage2[x1 + 2][y1]->getName())
			count = count + 1;
	}
	if (x1 - 1 >= 0 && blockStage2[x1][y1]->getName() == blockStage2[x1 - 1][y1]->getName()) {
		count = count + 1;
		if (x1 - 2 >= 0 && blockStage2[x1][y1]->getName() == blockStage2[x1 - 2][y1]->getName())
			count = count + 1;
	}
	...
	if (count >= 3)
		...
		count = 1;
		if (y1 + 1 < 8 && blockStage2[x1][y1]->getName() == blockStage2[x1][y1 + 1]->getName()) {
			count = count + 1;
			if (y1 + 2 < 8 && blockStage2[x1][y1]->getName() == blockStage2[x1][y1 + 2]->getName())
				count = count + 1;
		}
		if (y1 - 1 >= 0 && blockStage2[x1][y1]->getName() == blockStage2[x1][y1 - 1]->getName()) {
			count = count + 1;
			if (y1 - 2 >= 0 && blockStage2[x1][y1]->getName() == blockStage2[x1][y1 - 2]->getName())
				count = count + 1;
		}
		if (count >= 3)
			return true;
...
void BTypeGameScene::choseEffect(const int x2, const int y2)
{
...
void BTypeGameScene::explode(float dt)
{
	if (step > 0 && goal <= 0) {
		std::srand(time(NULL));
		int x = rand() % 8;
		...
		blockStage2[x][y]->setState(2);
		this->addChild(blockStage2[x][y]->getSpePic(), 1);

		for (int i = 0; i < 8; i++) {
			for (int j = 0; j < 8; j++) {
				if (blockStage2[i][j]->getState() == 2 || blockStage2[i][j]->getState() == 3) {
					for (int k = 0; k < 8; k++) {
						blockStage2[i][k]->getPic()->runAction(Sequence::create(ScaleTo::create(0.15f, 0.0f), NULL));
						blockStage2[i][k]->getSpePic()->runAction(Sequence::create(ScaleTo::create(0.15f, 0.0f), NULL));
						choseEffect(i, k);
						blockStage2[i][k]->setName("null");
						grade += pow(2, 9);
					}
					break;
				}
		...
		/*for (int i = 0; i < 8; i++) {
			blockStage2[x][i]->getPic()->runAction(Sequence::create(ScaleTo::create(0.15f, 0.0f), NULL));
			blockStage2[x][i]->getSpePic()->runAction(Sequence::create(ScaleTo::create(0.15f, 0.0f), NULL));
			choseEffect(x, i);
			blockStage2[x][i]->setName("null");
			grade += pow(2, 9);
		}*/
...
void BTypeGameScene::endGame(float dt)
{
	//全部消除，步数没有剩余
	if (step <= 0 && goal <= 0) {
		this->unschedule(schedule_selector(BTypeGameScene::clearBlocks));
		...
		initEndLayer(1);
		this->unschedule(schedule_selector(BTypeGameScene::endGame));
	}
	//没有步数,还没有消除完
	if (step <= 0 && goal > 0) {
		initEndLayer(2);
...
void BTypeGameScene::initEndLayer(const int result)
{
	auto vSize = Director::getInstance()->getVisibleSize();
	Vec2 origin = Director::getInstance()->getVisibleOrigin();
