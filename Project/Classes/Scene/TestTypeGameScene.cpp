...
//二维向量,元素舞台
std::vector<Block*> blockLine0;
std::vector<std::vector<Block*>> blockStage0;

Scene* TestTypeGameScene::createScene()
{
	return TestTypeGameScene::create();
}

bool TestTypeGameScene::init()
{
...
//静态元素
void TestTypeGameScene::initStillNode()
{
...
//动态元素
void TestTypeGameScene::initMoveLayer()
{
	auto vSize = Director::getInstance()->getVisibleSize();
	Vec2 origin = Director::getInstance()->getVisibleOrigin();
	...
	auto pauseItem = MenuItemImage::create("pic/PauseNormal.png", "pic/PauseSelected.png",
		CC_CALLBACK_1(TestTypeGameScene::getSetUpLayer, this, moveLayer));
...
//菜单元素
void TestTypeGameScene::initMenu(Node* pNode)
{
	auto vSize = Director::getInstance()->getVisibleSize();
	Vec2 origin = Director::getInstance()->getVisibleOrigin();
	...
	musicSlider->addTargetWithActionForControlEvents(this,
		cccontrol_selector(TestTypeGameScene::musicSliderCallBack), Control::EventType::VALUE_CHANGED);
	...
	effectSlider->addTargetWithActionForControlEvents(this,
		cccontrol_selector(TestTypeGameScene::effectSliderCallBack), Control::EventType::VALUE_CHANGED);
	pNode->addChild(effectSlider, 6);

	//动态菜单关闭按钮
	auto menuCloseItem = MenuItemImage::create("pic/MenuCloseNormal.png", "pic/MenuCloseSelected.png",
		CC_CALLBACK_1(TestTypeGameScene::menuClose, this, pNode));
	...
	auto returnGameItem = MenuItemImage::create("pic/ReturnHomeNormal.png", "pic/ReturnHomeSelected.png",
		CC_CALLBACK_1(TestTypeGameScene::returnCallBack, this));
	...
	auto closeItem = MenuItemImage::create("pic/CloseNormal2.png", "pic/CloseSelected2.png",
		CC_CALLBACK_1(TestTypeGameScene::gameCloseCallback, this));
...
//音乐滑块效果
void TestTypeGameScene::musicSliderCallBack(Ref* pSender, Control::EventType type)
{
...
//音效滑块效果
void TestTypeGameScene::effectSliderCallBack(Ref* pSender, Control::EventType type)
{
	auto slider = (ControlSlider*)pSender;
	auto value = slider->getValue();
	value = value / 100;
...
//鼠标点击关闭操作
void TestTypeGameScene::gameCloseCallback(Ref* pSender)
{
	blockLine0.clear();
	blockStage0.clear();
...
//鼠标点击弹出菜单
void TestTypeGameScene::getSetUpLayer(Ref* pSender, Node* pNode)
{
...
//鼠标点击关闭菜单
void TestTypeGameScene::menuClose(Ref* pSender, Node* pNode)
{
...
//返回上级场景
void TestTypeGameScene::returnCallBack(Ref* pSender)
{
	step = maxStep;
	grade = 0;
	blockLine0.clear();
...
/*********************************游戏逻辑部分*********************************/
void TestTypeGameScene::initGameLayer()
{
	...
	auto skillItem1 = MenuItemImage::create("pic/Skill1Normal.png", "pic/Skill1Selected.png",
		CC_CALLBACK_1(TestTypeGameScene::addStep, this));
	...
	auto skillItem2 = MenuItemImage::create("pic/Skill2Normal.png", "pic/Skill2Selected.png",
		CC_CALLBACK_1(TestTypeGameScene::switchTwo, this));
	...
	auto skillItem3 = MenuItemImage::create("pic/Skill3Normal.png", "pic/Skill3Selected.png",
		CC_CALLBACK_1(TestTypeGameScene::reFill, this));
	...
	for (int i = 0; i < 8; i++) {
		...
		blockStage0.push_back(blockLine0);
		blockLine0.clear();
	}

	//刷新
	this->schedule(schedule_selector(TestTypeGameScene::clearBlocks), 0.5f);
	this->schedule(schedule_selector(TestTypeGameScene::endGame), 3.7f);
	this->scheduleUpdate();
}

void TestTypeGameScene::addStep(cocos2d::Ref* pSender)
{
...
void TestTypeGameScene::switchTwo(cocos2d::Ref* pSender)
{
...
void TestTypeGameScene::reFill(cocos2d::Ref* pSender)
{
	if (UserDefault::getInstance()->getIntegerForKey("SkillThree") > 0) {
		...
		this->unschedule(schedule_selector(TestTypeGameScene::clearBlocks));

		for (int i = 0; i < 8; i++) {
			for (int j = 0; j < 8; j++) {
				blockStage0[i][j]->getPic()->runAction(ScaleTo::create(0.0f, 0.0f));
				blockStage0[i][j]->getSpePic()->runAction(ScaleTo::create(0.0f, 0.0f));
			}
		}
...
void TestTypeGameScene::update(float dt)
{
	...
	//鼠标点击第1次
	if (Block::flag == 1) {
		for (int i = 0; i < blockStage0.size(); i++) {
			for (int j = 0; j < blockStage0[i].size(); j++) {
				if (blockStage0[i][j]->getSwap() == 1) {
					MoveTo* move = MoveTo::create(0, blockStage0[i][j]->getPos());
	...
	if (Block::flag == 2) {
		...
		if ((numi[0] == numi[1] && abs(numj[0] - numj[1]) == 1) || (numj[0] == numj[1] && abs(numi[0] - numi[1]) == 1)) {
			cocos2d::MoveTo* move1 = cocos2d::MoveTo::create(0.05f, blockStage0[numi[0]][numj[0]]->getPos());
			cocos2d::MoveTo* move2 = cocos2d::MoveTo::create(0.05f, blockStage0[numi[1]][numj[1]]->getPos());

			blockStage0[numi[0]][numj[0]]->getPic()->runAction(Sequence::create(move2->clone(), DelayTime::create(0.05f), NULL));
			blockStage0[numi[0]][numj[0]]->getSpePic()->runAction(Sequence::create(move2->clone(), DelayTime::create(0.05f), NULL));
			blockStage0[numi[1]][numj[1]]->getPic()->runAction(Sequence::create(move1->clone(), DelayTime::create(0.05f), NULL));
			blockStage0[numi[1]][numj[1]]->getSpePic()->runAction(Sequence::create(move1->clone(), DelayTime::create(0.05f), NULL));

			Point po = blockStage0[numi[0]][numj[0]]->getPos();
			blockStage0[numi[0]][numj[0]]->setPos(blockStage0[numi[1]][numj[1]]->getPos());
			blockStage0[numi[1]][numj[1]]->setPos(po);

			blockStage0[numi[0]][numj[0]]->initSwap();
			blockStage0[numi[1]][numj[1]]->initSwap();

			std::swap(blockStage0[numi[0]][numj[0]], blockStage0[numi[1]][numj[1]]);

			if (!clearAble(numi[0], numj[0]) && !clearAble(numi[1], numj[1]) && isSwitchOn == false) {
				cocos2d::MoveTo* move3 = cocos2d::MoveTo::create(0.05f, blockStage0[numi[0]][numj[0]]->getPos());
				cocos2d::MoveTo* move4 = cocos2d::MoveTo::create(0.05f, blockStage0[numi[1]][numj[1]]->getPos());

				blockStage0[numi[0]][numj[0]]->getPic()->runAction(Sequence::create(DelayTime::create(0.05f), move4->clone(), NULL));
				blockStage0[numi[0]][numj[0]]->getSpePic()->runAction(Sequence::create(DelayTime::create(0.05f), move4->clone(), NULL));
				blockStage0[numi[1]][numj[1]]->getPic()->runAction(Sequence::create(DelayTime::create(0.05f), move3->clone(), NULL));
				blockStage0[numi[1]][numj[1]]->getSpePic()->runAction(Sequence::create(DelayTime::create(0.05f), move3->clone(), NULL));

				po = blockStage0[numi[0]][numj[0]]->getPos();
				blockStage0[numi[0]][numj[0]]->setPos(blockStage0[numi[1]][numj[1]]->getPos());
				blockStage0[numi[1]][numj[1]]->setPos(po);
...
void TestTypeGameScene::isTarget(const int x, const int y)
{
}

void TestTypeGameScene::clearBlocks(float dt)
{
	...
	auto clearOne = [&](int X, int Y) {
		blockStage0[X][Y]->getPic()->runAction(Sequence::create(ScaleTo::create(0.15f, 0.0f), NULL));
		blockStage0[X][Y]->getSpePic()->runAction(Sequence::create(ScaleTo::create(0.15f, 0.0f), NULL));
		isTarget(X, Y);
		choseEffect(X, Y);
		blockStage0[X][Y]->setName("null");
	};
	...
	for (int i = 0; i < 8; i++) {
		for (int j = 0; j < 8; j++) {
			...
			//判断有没有横着的(向右看)
			if (j < blockStage0[i].size() - 2) {
				if (blockStage0[i][j]->getName() == blockStage0[i][j + 1]->getName() && blockStage0[i][j]->getName() == blockStage0[i][j + 2]->getName()) {
					pushMeltPos(i, j);
					pushMeltPos(i, j + 1);
					pushMeltPos(i, j + 2);
					//4消
					if (j + 3 < blockStage0[i].size() && blockStage0[i][j]->getName() == blockStage0[i][j + 3]->getName()) {
						pushMeltPos(i, j + 3);
						//5消
						if (j + 4 < blockStage0[i].size() && blockStage0[i][j]->getName() == blockStage0[i][j + 4]->getName()) {
							pushMeltPos(i, j + 4);
						}
					}
					key = 1;
			...
			if (j - 2 >= 0) {
				if (blockStage0[i][j]->getName() == blockStage0[i][j - 1]->getName() && blockStage0[i][j]->getName() == blockStage0[i][j - 2]->getName()) {
					...
					pushMeltPos(i, j - 2);
					//4消
					if (j - 3 >= 0 && blockStage0[i][j]->getName() == blockStage0[i][j - 3]->getName()) {
						pushMeltPos(i, j - 3);
						//5消
						if (j - 4 >= 0 && blockStage0[i][j]->getName() == blockStage0[i][j - 4]->getName()) {
							pushMeltPos(i, j - 4);
						}
					}
					key = 1;
			...
			//判断有没有竖着的(向上看)
			if (i < blockStage0.size() - 2) {
				if (blockStage0[i][j]->getName() == blockStage0[i + 1][j]->getName() && blockStage0[i][j]->getName() == blockStage0[i + 2][j]->getName()) {
					...
					pushMeltPos(i + 2, j);
					//4消
					if (i + 3 < blockStage0.size() && blockStage0[i][j]->getName() == blockStage0[i + 3][j]->getName()) {
						pushMeltPos(i + 3, j);
						//5消
						if (i + 4 < blockStage0.size() && blockStage0[i][j]->getName() == blockStage0[i + 4][j]->getName()) {
							pushMeltPos(i + 4, j);
						}
					}
					key = 1;
			...
			if (i - 2 >= 0) {
				if (blockStage0[i][j]->getName() == blockStage0[i - 1][j]->getName() && blockStage0[i][j]->getName() == blockStage0[i - 2][j]->getName()) {
					...
					pushMeltPos(i - 2, j);
					//4消
					if (i - 3 >= 0 && blockStage0[i][j]->getName() == blockStage0[i - 3][j]->getName()) {
						pushMeltPos(i - 3, j);
						//5消
						if (i - 4 >= 0 && blockStage0[i][j]->getName() == blockStage0[i - 4][j]->getName()) {
							pushMeltPos(i - 4, j);
						}
					}
					key = 1;
	...
	if (maxMelti.size() != 0) {
		...
		if (maxMelti.size() == 3) {
			...
			int powerKey = 0;
			for (int i = 0; i < 3; i++) {
				if (blockStage0[maxMelti[i]][maxMeltj[i]]->getState() == 2) {
					blockStage0[maxMelti[i]][maxMeltj[i]]->setState(1);
					this->addChild(blockStage0[maxMelti[i]][maxMeltj[i]]->getSpePic(), 1);
					specialKey = 1;
				}
				if (blockStage0[maxMelti[i]][maxMeltj[i]]->getState() == 3) {
					blockStage0[maxMelti[i]][maxMeltj[i]]->setState(1);
					this->addChild(blockStage0[maxMelti[i]][maxMeltj[i]]->getSpePic(), 1);
					powerKey = 1;
				}
			...
			if (specialKey == 0 && powerKey == 0) {
				...
				for (int k = 0; k < maxMelti.size(); k++) {
					for (int i = 0; i < 8; i++) {
						if (blockStage0[maxMelti[k]][i]->getName() != "null") {
							clearOne(maxMelti[k], i);
						}
						grade += pow(2, 9);
					}
					for (int i = 0; i < 8; i++) {
						if (blockStage0[i][maxMeltj[k]]->getName() != "null") {
		...
		//4消
			...
			if (specialKey == 0 && powerKey == 0) {
				for (int k1 = 0; k1 < maxMelti.size(); k1++) {
					if (k1 == 1) {
						blockStage0[maxMelti[1]][maxMeltj[1]]->setState(2);
						this->addChild(blockStage0[maxMelti[1]][maxMeltj[1]]->getSpePic(), 1);
		...
		//5或更多消
			...
			if (specialKey == 0 && powerKey == 0) {
				for (int k1 = 0; k1 < maxMelti.size(); k1++) {
					if (k1 == 1) {
						blockStage0[maxMelti[1]][maxMeltj[1]]->setState(3);
						this->addChild(blockStage0[maxMelti[1]][maxMeltj[1]]->getSpePic(), 1);
						isTarget(maxMelti[1], maxMeltj[1]);
					}
					else {
...
void TestTypeGameScene::fillFull()
{
	//向下补齐
	for (int i = 0; i < 8; i++) {
		for (int j = 0; j < 8; j++) {
			if (blockStage0[j][i]->getName() == "null") {
				for (int k2 = j; k2 < 8; k2++) {
					if (blockStage0[k2][i]->getName() == "null") {
						continue;
					}
					else {
						Point tempPos = blockStage0[k2][i]->getPos();
						blockStage0[k2][i]->setPos(blockStage0[j][i]->getPos());
						blockStage0[j][i]->setPos(tempPos);

						std::swap(blockStage0[k2][i], blockStage0[j][i]);

						blockStage0[j][i]->getPic()->runAction(
							Sequence::create(MoveTo::create(0.15f, pos[j][i]), NULL));
						blockStage0[j][i]->getSpePic()->runAction(
							Sequence::create(MoveTo::create(0.15f, pos[j][i]), NULL));
						blockStage0[k2][i]->getPic()->runAction(
							Sequence::create(MoveTo::create(0.15f, pos[k2][i]), NULL));
						blockStage0[k2][i]->getSpePic()->runAction(
							Sequence::create(MoveTo::create(0.15f, pos[j][i]), NULL));
	...
	for (int i = 0; i < 8; i++) {
		for (int j = 0; j < 8; j++) {
			if (blockStage0[i][j]->getName() == "null") {
				picNum = rand() % 6 + 1 + 6 * (UserDefault::getInstance()->getIntegerForKey("Skin") - 1);
				sprintf(picName, "block/llk%d.png", picNum);
				blockStage0[i][j]->setName(picName);
				blockStage0[i][j]->resetPic(picName, spos[j]);

				blockStage0[i][j]->getPic()->runAction(
					Sequence::create(MoveTo::create(0.15f, pos[i][j]), NULL));
				blockStage0[i][j]->getSpePic()->runAction(
					Sequence::create(MoveTo::create(0.15f, pos[i][j]), NULL));

				auto menu = Menu::create(blockStage0[i][j]->getPic(), NULL);
				menu->setPosition(Point::ZERO);
				this->addChild(menu, 1);
				this->addChild(blockStage0[i][j]->getSpePic(), 1);
			}
		}
	}
}

bool TestTypeGameScene::clearAble(const int x1, const int y1)
{
	int count = 1;
	if (x1 + 1 < 8 && blockStage0[x1][y1]->getName() == blockStage0[x1 + 1][y1]->getName()) {
		count = count + 1;
		if (x1 + 2 < 8 && blockStage0[x1][y1]->getName() == blockStage0[x1 + 2][y1]->getName())
			count = count + 1;
	}
	if (x1 - 1 >= 0 && blockStage0[x1][y1]->getName() == blockStage0[x1 - 1][y1]->getName()) {
		count = count + 1;
		if (x1 - 2 >= 0 && blockStage0[x1][y1]->getName() == blockStage0[x1 - 2][y1]->getName())
			count = count + 1;
	}
	...
	if (count >= 3)
		...
		count = 1;
		if (y1 + 1 < 8 && blockStage0[x1][y1]->getName() == blockStage0[x1][y1 + 1]->getName()) {
			count = count + 1;
			if (y1 + 2 < 8 && blockStage0[x1][y1]->getName() == blockStage0[x1][y1 + 2]->getName())
				count = count + 1;
		}
		if (y1 - 1 >= 0 && blockStage0[x1][y1]->getName() == blockStage0[x1][y1 - 1]->getName()) {
			count = count + 1;
			if (y1 - 2 >= 0 && blockStage0[x1][y1]->getName() == blockStage0[x1][y1 - 2]->getName())
				count = count + 1;
		}
		if (count >= 3)
			return true;
...
void TestTypeGameScene::choseEffect(const int x2, const int y2)
{
...
void TestTypeGameScene::endGame(float dt)
{
	//全部消除，步数没有剩余
	if (step <= 0) {
		...
		initEndLayer(1);
		this->unschedule(schedule_selector(TestTypeGameScene::endGame));
	}
}

void TestTypeGameScene::initEndLayer(const int result)
{
	auto vSize = Director::getInstance()->getVisibleSize();
	Vec2 origin = Director::getInstance()->getVisibleOrigin();
